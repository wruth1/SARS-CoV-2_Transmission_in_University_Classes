...
# global_seed = parse(Int, ARGS[1])
# Random.seed!(global_seed)
Random.seed!(1)
M = 1 # Number of times to replicate each parameter combination
...
@load "Data/Objects/Thin_Status_Raw.jld2"    # Status object without risks
...
if !@isdefined all_thresholds; const all_thresholds = [∞]; end # Maximum class sizes
...
if (!@isdefined status_raw)
    status_raw = thin_data("Data/2019-Fall.csv", target_enrollments)    # 32008 enrollments to match 
    delete_tiny_classes!(status_raw)

    # ------------------ Extract the largest connected component ----------------- #
    delete_isolated_components!(status_raw);


    @save "Data/Objects/Thin_Status_Raw.jld2" status_raw
end
...
this_threshold = all_thresholds[1]
this_status = status_raw
...
@save "Data/Objects/Thinned/M=$M, seed=$global_seed, N=$N.jld2"  all_sim_outputs all_parameters
