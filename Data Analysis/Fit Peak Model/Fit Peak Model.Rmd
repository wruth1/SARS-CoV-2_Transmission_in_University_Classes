...
```{r group_peak_hist, fig.cap="\\label{fig:group_hist}Histograms of peak size for each class size threshold with uniform axis scaling."}
...
```{r group_peak_hist_free, fig.cap="\\label{fig:group_hist_free}Histograms of peak size for each class size threshold with heteogeneous axis scaling."}
...
jpeg("C:\\Users\\willi\\sfuvault2\\CornellStudy\\Tex\\Plots\\Peak_Size_Analysis\\Peak_Hist_Homo.jpeg")
plot(peak_hist_group)
dev.off()
```
...
jpeg("C:\\Users\\willi\\sfuvault2\\CornellStudy\\Tex\\Plots\\Peak_Size_Analysis\\Peak_Hist_Hetero.jpeg")
plot(peak_hist_group_free)
dev.off()
```
...
jpeg("C:\\Users\\willi\\sfuvault2\\CornellStudy\\Tex\\Plots\\Peak_Size_Analysis\\Peak_Disper_Ref.jpeg")
plot(pure_rep_plot_ref)
dev.off()
```
...
jpeg("C:\\Users\\willi\\sfuvault2\\CornellStudy\\Tex\\Plots\\Peak_Size_Analysis\\Peak_Disper_Ref_Scaled.jpeg")
plot(pure_rep_plot_ref_scaled)
dev.off()
```
...
jpeg("C:\\Users\\willi\\sfuvault2\\CornellStudy\\Tex\\Plots\\Peak_Size_Analysis\\Peak_Boxplots.jpeg")
grid.arrange(grobs = all_boxplots, ncol=3)
dev.off()
```
